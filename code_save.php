...
try{
	//検索処理
	$stmt = $pdo->prepare('SELECT * FROM amusers LEFT JOIN amschedule
			ON amusers.user_ID = amschedule.schedule_user_ID
			WHERE schedule_date BETWEEN :month_fast_day AND :next_month_fast_day
			ORDER BY schedule_date asc ,user_ID');
...
$workers_array = array();//当日の勤務者名を入れる配列初期化
while($row = $stmt -> fetch(PDO::FETCH_ASSOC)) { /*スタッフの人数の一ヶ月の勤務日分繰り返す */
	$workers_array[$row[schedule_date]] .= "<br/>".$row[user_display_name];//一行改行、勤務者名を代入
}
?>
...
?>
	...
	>
		...
		if($display_calendar_flag == 1){//flagが1だったら、下記の処理を実行
			...
			.display_to_Holidays(date("Y-m-d",strtotime($display_year."-".$display_month."-".$display_day_array[$ii])),$Holidays_array)
			.display_to_workers(date("Y-m-d",strtotime($display_year."-".$display_month."-".$display_day_array[$ii])),$workers_array);; //祝日があれば一行改行して表示、function.phpの中の関数
			$i++;
			$ii++;
